  ...
  io.on('connection', (socket) => {
    socket.on('create_room', ({ name, avatar, partyCode } = {}, ack) => {
      ...
      const room = {
        code,
        hostId: playerId,
        players: [{ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), connected: true, disconnectTimer: null }],
    ...
    socket.on('join_room', ({ code, name, avatar } = {}, ack) => {
      ...
      const playerId = makePlayerId();
      const sessionToken = makeSessionToken();
      room.players.push({ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), connected: true, disconnectTimer: null });
