...
const { LOCATION_CATEGORIES, CHARACTER_CATEGORIES, CHARACTER_MIX_DECOY_GROUPS, PLACES_MIX_DECOY_GROUPS } = require('../data');
...
function registerSpyGame(io) {
  ...
  // другая, похожая тема — так что он тоже не знает, что он шпион.
  function decoyGroupsFor(category, subCategory) {
    if (subCategory !== 'mix') return null; // курация есть только для "Микса" — см. комментарий в data.js
    return category === 'characters' ? CHARACTER_MIX_DECOY_GROUPS : PLACES_MIX_DECOY_GROUPS;
  }
