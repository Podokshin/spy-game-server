...
const { DISCONNECT_GRACE_MS, sanitizeAvatar, makeRoomCode, makePlayerId, makeSessionToken } = require('./shared');
  ...
  io.on('connection', (socket) => {
    socket.on('create_room', ({ name, avatar, partyCode } = {}, ack) => {
      ...
      const room = {
        code,
        hostId: playerId,
        players: [{ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), score: 0, connected: true, disconnectTimer: null, color: null }],
    ...
    socket.on('join_room', ({ code, name, avatar } = {}, ack) => {
      ...
      const playerId = makePlayerId();
      const sessionToken = makeSessionToken();
      room.players.push({ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), score: 0, connected: true, disconnectTimer: null, color: null });
