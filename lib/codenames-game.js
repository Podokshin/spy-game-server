...
function registerCodenamesGame(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('guess_card', ({ index } = {}) => {
      ...
      if (card.color === team) {
        // Угадали свою карту — лимит на число попыток не действует, команда может
        // продолжать угадывать сколько угодно раз подряд, пока не ошибётся или сама
        // не решит закончить ход кнопкой «Завершить ход».
