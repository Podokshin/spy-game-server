...
const ROLES = {
  skuf: { name: 'Скуф', icon: '🧢', desc: 'Если его свидание этой ночью — Альтушка, получает +2 сердца вместо +1.' },
  altushka: { name: 'Альтушка', icon: '🖤', desc: 'Если её пару этой ночью больше вообще никто не выбрал — получает +1 сердце.' },
  pikmi: { name: 'Пикми', icon: '🙄', desc: 'Если ей написали 3 и более разных игрока за ночь — получает +1 сердце.' },
  gopnik: { name: 'Гопник', icon: '🧃', desc: 'При удачном свидании отжимает 1 сердце у того, с кем сам заматчился.' },
  uspeh: { name: 'Успешный успех', icon: '💼', desc: 'По нечётным ночам сердца от свидания удваиваются. По чётным без свидания — доп. штраф -1.' },
  zozh: { name: 'ЗОЖница', icon: '🥦', desc: 'Если 2 ночи подряд без единого свидания — теряет 1 сердце.' },
  streamer: { name: 'Задрот-стример', icon: '🎮', desc: 'Если какой-то игрок потратил на него все свои 4 сообщения этой ночи — получает +1 сердце.' },
  vanilla: { name: 'Ванильная штучка', icon: '🎀', desc: 'Если её отвергли — получает +1 сердце, но только один раз за всю игру.' },
  intriganka: { name: 'Интриганка', icon: '🐍', desc: 'Если её выбрали, а она выбрала кого-то другого — забирает 1 сердце у отвергнутого.' },
  mlm: { name: 'МЛМ-щица', icon: '💊', desc: 'При удачном свидании «вербует» партнёра — тот тоже вербует при своих удачных свиданиях. Если к концу игры завербованы все — побеждает мгновенно, несмотря на сердца.' },
};
...
function registerSkufGame(io) {
  ...
  // Пары, которые заматчились этой ночью — каждая пара один раз (не дважды
  // в обе стороны). Порядок используется для поэтапного раскрытия: сначала
  // общая картина, потом переписка каждой пары по очереди, потом итог.
  function computeMatchedPairs(players) {
    const pairs = [];
    const seen = new Set();
    players.forEach(p => {
      if (p.matchedWith && !seen.has(p.id) && !seen.has(p.matchedWith)) {
        seen.add(p.id);
        seen.add(p.matchedWith);
        pairs.push([p.id, p.matchedWith]);
      }
    });
    return pairs;
  }
  ...
  function buildRevealStepPayload(room) {
    ...
    const pairs = reveal.matchedPairs;
    ...
    if (stepIndex <= pairs.length) {
      const [aId, bId] = pairs[stepIndex - 1];
      ...
      return {
        ...
        b: { id: b.id, name: b.name, avatar: b.avatar, delta: b.delta },
        messages,
        infectedA: reveal.newlyInfected.includes(aId),
        infectedB: reveal.newlyInfected.includes(bId)
      };
    }
    return {
      ...base,
  ...
  function finalizePicking(room, timeUp) {
    ...
    room.game.lastReveal = {
      ...
      players,
      matchedPairs: computeMatchedPairs(players),
      revealedRole,
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('next_reveal_step', () => {
      ...
      const totalSteps = room.game.lastReveal.matchedPairs.length + 2;
