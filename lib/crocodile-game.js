...
const { DISCONNECT_GRACE_MS, sanitizeAvatar, makeRoomCode, makePlayerId, makeSessionToken, shuffle } = require('./shared');
  ...
  io.on('connection', (socket) => {
    socket.on('create_room', ({ name, avatar, partyCode } = {}, ack) => {
      ...
      const room = {
        code,
        hostId: playerId,
        players: [{ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), score: 0, connected: true, disconnectTimer: null }],
        settings: { totalRounds: 6, roundSeconds: 80 },
        phase: 'lobby', // lobby | choosing | drawing | reveal | end | skipped
        game: null,
        round: 0,
        artistTurnCount: 0,
        usedWords: new Set(),
      ...
      ack && ack({ ok: true, ...roomSummary(room), playerId, sessionToken });
    });
    ...
    socket.on('join_room', ({ code, name, avatar } = {}, ack) => {
      ...
      const playerId = makePlayerId();
      const sessionToken = makeSessionToken();
      room.players.push({ id: playerId, sessionToken, socketId: socket.id, name: playerName, avatar: sanitizeAvatar(avatar), score: 0, connected: true, disconnectTimer: null });
    ...
    socket.on('rejoin', ({ roomCode, playerId, sessionToken } = {}, ack) => {
      ...
      if (!room) return ack && ack({ ok: false, error: 'Комната не найдена' });
      const player = room.players.find(p => p.id === playerId);
      if (!player || player.sessionToken !== sessionToken) return ack && ack({ ok: false, error: 'Вы не были в этой комнате' });
      ...
      if (room.phase === 'end') {
        payload.ended = { players: publicPlayers(room), partyStandings: party.getStandings(room.code) };
      }
