...
//
// В длинных нардах нет взятия шашек — на одной точке может находиться любое
// количество шашек обоих цветов одновременно, поэтому ходы не блокируются
// чужими шашками (в отличие от классического бэкгаммона).
...
function hasAnyLegalMove(board, color, diceValues) {
  const uniqueDice = Array.from(new Set(diceValues));
  return uniqueDice.some(die => listLegalSources(board, color, die).length > 0);
}

// Применяет ход (мутирует board). Возвращает { to } (null, если шашка вышла).
function applyMove(board, color, from, die) {
  const move = describeMove(board, color, from, die);
