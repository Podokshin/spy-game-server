  ...
  function startPlaying(room) {
    ...
    if (room.timer.enabled) {
      room.timerHandle = setTimeout(() => {
        if (room.phase === 'playing') finishGame(room, true);
      }, totalMs);
    }
  }

  function finishGame(room, timeUp) {
    clearRoomTimer(room);
    room.phase = 'end';
  ...
  io.on('connection', (socket) => {
    socket.on('create_room', ({ name, avatar } = {}, ack) => {
      ...
      const room = {
        ...
        settings: { timerEnabled: true, timerMinutes: 12 },
    ...
    socket.on('update_settings', (settings = {}) => {
      ...
      room.settings = {
        ...
        timerMinutes: Math.min(30, Math.max(1, parseInt(settings.timerMinutes, 10) || 12))
      };
      broadcastRoom(room);
    });
    ...
    socket.on('toggle_pause', () => {
      const room = getRoom(socket);
      if (!room || !isHost(room, socket) || room.phase !== 'playing' || !room.timer.enabled) return;
