...
const SESSION_KEY = 'wall_online_session_v1'
...
const DEFAULT_MENU_SUBTITLE = 'Каждый раунд все анонимно пишут признание на общую тему — угадайте, кто есть кто, а автору — очки за удачную маскировку.'
const INVITE_MENU_SUBTITLE = 'Вас пригласили сыграть! Впишите имя, выберите аватар и присоединяйтесь.'

export function useWallGame() {
  ...
  const [confessionProgress, setConfessionProgress] = useState({ count: 0, total: 0 })

  const [wallStep, setWallStep] = useState(null)
  const [myGuess, setMyGuess] = useState(undefined)
  ...
  function renderWritingScreen(data) {
    playRoundStartSound()
    setWritingData(data)
    setMySubmitted(!!data.alreadySubmitted)
    setConfessionProgress({ count: 0, total: liveRef.current.currentRoom ? liveRef.current.currentRoom.players.length : 0 })
    setScreen('writing')
  }

  function renderWallStep(data) {
    if (data.kind === 'result') playResultSound()
    setWallStep(data)
    if (data.kind === 'vote') {
      setMyGuess(undefined)
      setGuessSubmitted(!!data.alreadyVoted)
    }
    setScreen('reveal')
  }
  ...
  useEffect(() => {
    function attemptRejoin() {
      ...
      socket.emit('rejoin', saved, (res) => {
        ...
        if (res.phase === 'writing' && res.writing) {
          renderWritingScreen(res.writing)
        } else if (res.phase === 'reveal' && res.step) {
          renderWallStep(res.step)
        } else if (res.phase === 'end') {
          renderEndScreen({ players: res.players, partyStandings: res.partyStandings })
    ...
    function onRoundWritingStarted(data) {
      renderWritingScreen(data)
    }

    function onConfessionProgress({ count, total }) {
      setConfessionProgress({ count, total })
    }

    function onWallStep(data) {
      renderWallStep(data)
    }

    function onVoteProgress({ count, total }) {
    ...
    socket.on('round_writing_started', onRoundWritingStarted)
    socket.on('confession_progress', onConfessionProgress)
    socket.on('wall_step', onWallStep)
    ...
    return () => {
      ...
      socket.off('round_writing_started', onRoundWritingStarted)
      socket.off('confession_progress', onConfessionProgress)
      socket.off('wall_step', onWallStep)
  ...
  function submitConfession(text) {
    if (!text || !text.trim() || mySubmitted) return
    setMySubmitted(true)
    socket.emit('submit_confession', { text: text.trim() })
  }

  function forceFinishWriting() {
    socket.emit('force_finish_writing')
  }

  function submitGuess(guessedId) {
    if (guessSubmitted) return
    setMyGuess(guessedId ?? null)
    setGuessSubmitted(true)
    socket.emit('submit_guess', { guessedId: guessedId || undefined })
  }

  function forceFinishVote() {
  ...
  return {
    ...
    writingData, mySubmitted, submitConfession, confessionProgress, forceFinishWriting,

    wallStep, myGuess, submitGuess, guessSubmitted, guessProgress, forceFinishVote, nextStep,
